...
# Script pour builder l'APK Release signé

Write-Host "🚀 Génération de l'APK Release Signé (RétroBus Essonne)" -ForegroundColor Cyan
...
$keystorePath = ".\retrobus-essonne.keystore"
if (-not (Test-Path $keystorePath)) {
    ...
    keytool -genkey -v -keystore $keystorePath `
        -keyalg RSA -keysize 2048 -validity 10000 -alias retrobus_key `
        -storepass $password -keypass $password `
        -dname "CN=RétroBus Essonne, O=Association, L=Essonne, C=FR"
...
if (Test-Path $apkPath) {
    ...
    Write-Host "   • Télécharger sur Google Play Store" -ForegroundColor Gray
    Write-Host "   • Ou distribuer manuellement aux utilisateurs" -ForegroundColor Gray
}
